...
async fn main() -> Result<()> {
    ...
    println!("✅ Konfiguracja załadowana");
    info!("✅ Konfiguracja załadowana");

    // 🚀 Inicjalizacja komponentów
    println!("🚀 Inicjalizacja silnika egzekucji...");
    let execution_engine = Arc::new(ExecutionEngine::new(config.clone()).await?);
    println!("📊 Inicjalizacja metryki...");
    let metrics = Arc::new(MetricsCollector::new());

    println!("✅ Silnik egzekucji zainicjalizowany");
